...
class MusicDomain:
    @classmethod
    def queryset(cls):
        return Music.objects.select_related("author").prefetch_related("like")

    @classmethod
    def get(cls, ulid: str, publish: bool) -> Music | None:
        return cls.queryset().filter(ulid=ulid, publish=publish).first()

    @classmethod
    def bulk_get(cls, filter: FilterOption, exclude: ExcludeOption, sort: SortOption, limit: int | None) -> list[Music]:
        q_list: list[Q] = []
        e_list: list[Q] = []
        if filter.publish:
    ...
    @classmethod
    def like(cls, obj: Music, user: User) -> bool:
