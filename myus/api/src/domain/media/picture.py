...
class PictureDomain:
    ...
    @classmethod
    def get(cls, ulid: str, publish: bool) -> Picture | None:
        return cls.queryset().filter(ulid=ulid, publish=publish).first()

    @classmethod
    def bulk_get(cls, filter: FilterOption, exclude: ExcludeOption, sort: SortOption, limit: int | None) -> list[Picture]:
        q_list: list[Q] = []
        e_list: list[Q] = []
        if filter.publish:
