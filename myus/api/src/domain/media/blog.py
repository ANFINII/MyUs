...
class BlogDomain:
    @classmethod
    def queryset(cls):
        return Blog.objects.select_related("author").prefetch_related("like")

    @classmethod
    def get(cls, ulid: str, publish: bool) -> Blog | None:
        return cls.queryset().filter(ulid=ulid, publish=publish).first()

    @classmethod
    def bulk_get(cls, filter: FilterOption, exclude: ExcludeOption, sort: SortOption, limit: int | None) -> list[Blog]:
        q_list: list[Q] = []
        e_list: list[Q] = []
        if filter.publish:
    ...
    @classmethod
    def like(cls, obj: Blog, user: User) -> bool:
