...
from api.src.domain.interface.comment.data import CommentData as DomainCommentData
...
def get_comments(type_no: CommentTypeNo, object_id: int, user_id: int | None) -> list[CommentData]:
    ...
    reply_map: dict[int, list[DomainCommentData]] = {}
    for r in replies:
...
def create_comment(user_id: int, input: CommentCreateIn) -> CommentData | None:
    ...
    saved_ids = comment_repo.bulk_save([new_comment])
    saved = comment_repo.bulk_get(saved_ids)[0]
    ...
    return CommentData(
        ...
        author=get_author(user_map, saved.author_id),
        replys=[],
    )
...
def update_comment(comment_ulid: str, text: str) -> None:
    comment_repo = CommentRepository()
    ids = comment_repo.get_ids(FilterOption(ulid=comment_ulid), SortOption())
    if len(ids) == 0:
        log.warning("コメントが見つかりませんでした")
        return None

    comment = comment_repo.bulk_get(ids)[0]
    updated = replace(comment, text=text)
    comment_repo.bulk_save([updated])
    return None
...
def delete_comment(comment_ulid: str) -> None:
    ...
    comment = comment_repo.bulk_get(ids)[0]
    updated = replace(comment, deleted=True)
    comment_repo.bulk_save([updated])
    return None
